const day7Input = `muted lime bags contain 1 wavy lime bag, 1 vibrant green bag, 3 light yellow bags.
...
//   const newbag = rule.split(' contain ')[0].split(' bags')[0];
//   const containbags = rule.split(' contain ')[1].split(', ');
//   containbags.forEach((bag) => {
//     if (bag[0] !== 'n') {
//       const color = bag.slice(2).split(' bag')[0];
//       if (rulesObj[color]) {
//         rulesObj[color].push(newbag);
//       } else {
//         rulesObj[color] = [newbag];
//       }
//     }
//   });
// }
...
//   if (object[query]) {
//     for (const color of object[query]) {
//       search(color, object);
//       if (res.includes(color) !== true) {
//         res.push(color);
//       }
//     }
//   }
// };
// search('shiny gold', rulesObj);
...
for (const rule of rules) {
  ...
  containbags.forEach((bag) => {
    if (bag[0] !== 'n') {
      const number = parseInt(bag[0]);
      const color = bag.slice(2).split(' bag')[0];
      newObj[color] = number;
    }
  });
  inputObj[newbag] = newObj;
}

const calcObj = {};

const calcBag = (query, object) => {
  const qObj = object[query];
  let res = 0;
  if (!qObj) {
    return 1;
  }
  for (const color in qObj) {
    res += qObj[color] + qObj[color] * calcBag(color, object);
  }
  return res;
};
res = calcBag('shiny gold', inputObj);
console.log(res);
