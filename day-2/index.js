...
// const check = (acc, el) => {
//   const test = el.password.split('');
//   const num = test.reduce((acc, current) => {
//     if (current === el.key) {
//       acc++;
//     }
//     return acc;
//   }, 0);
//   if (num >= el.min && num <= el.max) {
//     return acc + 1;
//   } else {
//     return acc;
//   }
...
const check2 = (acc, el) => {
  const test = el.password.split('');
  if (test[el.firstPosition] === el.key && test[el.secondPosition] !== el.key) {
    return acc + 1;
  } else if (
    test[el.firstPosition] !== el.key &&
    test[el.secondPosition] === el.key
  ) {
    return acc + 1;
  } else {
    return acc;
